...
c
      IMPLICIT NONE
      include '/home/seraf/hGAST/post_hGAST.dir/paramet.for'
...
c     Local variables
      ...
      CHARACTER*500 title
      CHARACTER*500 inpstr, inpgeo, inpload
      CHARACTER*300 n_p, n_d
      CHARACTER*600 n_t
      CHARACTER*500 p1, p2, p3, p4, p5
c
c     ... input files
      READ(*,"(a)") n_p
      READ(*,"(a)") n_d
      n_t=trim(adjustl(n_p))//"/"//trim(adjustl(n_d))
      inpgeo=trim(n_t)//"/other.dir/geometry.dat"   ! READ (*, '(a)') inpgeo                 !=========================================> seraf changes
      inpstr=trim(n_t)//"/other.dir/structury.dat"  ! READ (*, '(a)') inpstr                 !
      OPEN (1, FILE=inpgeo, STATUS='old')
      OPEN (2, FILE=inpstr, STATUS='old')
c
      p1=trim(n_t)//"/other.dir/section.out"
...
c       
     ...
     &               lpseg(1:mseg,1:mloops,isct), nlpsg(1:mloops,isct), 
     &                 corweb,0.d0,1.d0,rshr(1),qshear)
c   
         CALL SHEARFLO(isct, a , b, A0, B0, D0, xpnt, ypnt,  
     &                 nskseg(isct), lamsg(1:mseg,isct), 
     &                 isgpt(1:2,1:mseg,isct), sknseg,  
     &                 nwebs(isct),webseg, nloops(isct), 
     &               lpseg(1:mseg,1:mloops,isct), nlpsg(1:mloops,isct), 
     &                 corweb,-1.d0,0.d0,rshr(2),qshear)
...
c               CALL offply1(im(ip,ilam), ip, angl(ip,ilam),cl,cc)
     ...
     &                             cc(i, 3, ip)
        ...
        p2=trim(adjustl(n_t))//"/other.dir/section_matrix_hGAST.dat"
 ...
 735    FORMAT(6(F30.5,3X))                              !   
        ...
        p3="/home/seraf/hGAST/post_hGAST.dir/elstif.dat"
...
c
     ...
     &          'ENTER O to STOP or 1 to continue with strains/stresses'
         icase=1   ! READ (*, *) icase   !===============================================> seraf changes
         IF ( icase==1 ) THEN
            WRITE (*, *) '  ENTER LOADS FILE NAME'
            p4=trim(adjustl(n_t))//"/other.dir/loads.dat" !READ (*, '(a)') inpload
...
c
     ...
     &       aahstr(mloops),rzeta,rs,area(mseg),aaloop(4),aaa(4, 4)
      ...
      CHARACTER*600 p
c
      p="/home/seraf/hGAST/post_hGAST.dir/test.txt"
      open(5,file="test.txt",status='unknown')
c
      DO i = 1, 3
         DO j = 1, 3
            a0(i, j) = 0.
...
c
     ...
     &                   im, strength,fcrit1,fcrit2,corweb, n_t)
      IMPLICIT NONE
...
c    
      path=trim(adjustl(n_t))//"/other.dir/stresses.dat"
      OPEN (18, FILE=path, STATUS='unknown')
      path=trim(adjustl(n_t))//"/other.dir/stresses_tswu.dat"
      OPEN(UNIT=44, FILE=path)                                !===================================> seraf changes
      path=trim(adjustl(n_t))//"/other.dir/stresses_norm.dat"
      OPEN(UNIT=45, FILE=path)                                !===================================> seraf changes
      path=trim(adjustl(n_t))//"/other.dir/stresses_peri.dat"
...
c
     ...
     &                 nskseg,lamsg,isgmt,sknseg,nwebs, webseg, 
     &                 nloop, lpseg, nlpsg, corweb,QX0,QY0,To0,q2)
      IMPLICIT NONE
      include '/home/seraf/hGAST/post_hGAST.dir/paramet.for'
...
c     ...invert stiffness to calculate section compliance
      ...
      p="/home/seraf/hGAST/post_hGAST.dir/seccmp.dat"
      open (1,file=p,status='unknown')
      DO i = 1, 3
         DO j = 1, 3
            seccmp(i  , j  ) = A0(i, j)
            seccmp(i  , j+3) = B0(i, j)
