...
do i=1,30
   ...
   kp(i,11)=kp_11(i)+caps_move(i) ; if (kp(i,11)>=(kp(i,12)-1)) kp(i,11)=kp(i,12)-1
               th=0.0 !(twist_or(i)+twist_re(i)-90.0)*4.0*atan(1.0)/180.0
...
write(*,*) gra1, gra2, (180.0/(4.0*atan(1.0)))*atan((gra2-gra1)/(1.0+gra2*gra1))
end do

!     do i=1,30
!        ! find maximum legth kp-7 to kp-8 or kp-11 to kp-12
!        !--->
!           s_max1=0.0
!           do j=kp_7(i),(kp(i,8)-1)
!              th=(twist_or(i)+twist_re(i)-90.0)*4.0*atan(1.0)/180.0
!              s_max1=s_max1+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!           end do
!           s_max2=0.0
!           do j=kp_11(i),(kp(i,12)-1)
!              th=(twist_or(i)+twist_re(i)-90.0)*4.0*atan(1.0)/180.0
!              s_max2=s_max2+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!           end do
!        !--->

!        if (s_max1<s_max2) then
!           s_max=s_max1
!           ! re-define kp-7
!           !--->
!              d_min=1000.0 ; s=0.0
!              do j=kp_7(i),(kp(i,8)-1)
!                 s=s+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!                 if (abs(s/s_max-caps_mov(i))<d_min) then
!                    d_min=abs(s/s_max-caps_mov(i)) ; kp(i,7)=j
!                 end if
!              end do
!           !--->

!           ! find legth kp_7 to kp-7
!           !--->
!              d=0.0
!              do j=kp_7(i),kp(i,7)
!                 th=(twist_or(i)+twist_re(i)-90.0)*4.0*atan(1.0)/180.0
!                 d=d+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!              end do
!           !--->
...
!              do j=kp_6(i),kp(i,7)
!                 s=s+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!                 if (abs(s-d)<d_min) then
!                    d_min=abs(s-d) ; kp(i,6)=j
!                 end if
!              end do
!           !--->
...
!           !--->
!              d_min=1000.0 ; s=0.0
!              do j=kp_11(i),kp(i,12)
!                 s=s+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!                 if (abs(s-d)<d_min) then
!                    d_min=abs(s-d) ; kp(i,11)=j
!                 end if
!              end do
!           !--->
...
!           !--->
!              d_min=1000.0 ; s=0.0
!              do j=kp_10(i),kp(i,11)
!                 s=s+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!                 if (abs(s-d)<d_min) then
!                    d_min=abs(s-d) ; kp(i,10)=j
!                 end if
!              end do
!           !--->
!        else
...
!                 s=s+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!                 if (abs(s/s_max-caps_mov(i))<d_min) then
!                    d_min=abs(s/s_max-caps_mov(i)) ; kp(i,11)=j
!                 end if
!              end do
!           !--->

!           ! find legth kp_11 to kp-11
!           !--->
!              d=0.0
!              do j=kp_11(i),kp(i,11)
!                 th=(twist_or(i)+twist_re(i)-90.0)*4.0*atan(1.0)/180.0
!                 d=d+sqrt((chord(i)*(air((j+1),1,i)*cos(th)-air((j+1),2,i)*sin(th))-chord(i)*(air(j,1,i)*cos(th)-air(j,2,i)*sin(th)))**2.0+(chord(i)*(air((j+1),1,i)*sin(th)+air((j+1),2,i)*cos(th))-chord(i)*(air(j,1,i)*sin(th)+air(j,2,i)*cos(th)))**2.0)
!              end do
!           !--->
