...
!
! library   : lib_ctrl
! author    : Dimitris Manolas
! institute : National Technical University of Athens (NTUA)
!
!
! Implimentation of a generic state-of-the-art control library for 
! variable speed / variable pitch WTs
! The library is based on Bossanyi's UPWIND (2009) and DTU's INNWIND (2013) controllers.
! 
! The library is initiated by calling the subroutine "lib_ctrl_init".
!
! The subroutine "lib_ctrl_main" is called every time and iteration step, while
! "lib_ctrl_update" at the end of every timestep.
!
!---------------------------------------------------------------------------!
 module lib_ctrl
...
!--- General Variables
   real(8), save :: TIME, DT_el, TPERIOD_el, R2D, PI2, RAT_GEAR
   integer, save :: NBLADE

!--- Control Variables
   integer, parameter :: NQSC0 = 55    ! number of Control Variables
   real(8), save :: QTC_el (NQSC0), QTC1_el (NQSC0), QTC2_el (NQSC0)
   real(8), save :: QTCP_el(NQSC0), QTCP1_el(NQSC0), QTCP2_el(NQSC0)

!--- Output vars
   real(8), save :: TGenLSS           ! Generator actual Torque at LSS
   real(8), save :: TGenLSS_D         ! Generator Damping term through linearization
   real(8), save :: TGenLSS_M         ! Generator Inertia term through linearization of DTD and Gen Lag
   real(8), save :: pitchOUT(3), dpitchOUT(3), ddpitchOUT(3)
   real(8), save ::  flapOUT(3),  dflapOUT(3),  ddflapOUT(3)
   real(8), save :: TMLossLSS         ! Torque due to mechanichal loss
   real(8), save :: TBrakeLSS         ! Actural Braque torque
!  real(8), save :: TIME_BRAKE        ! Time at which brake was initialized


!--- Sensors
   real(8), save :: GenSpeed   , dGenSpeed
   real(8), save :: BlPitch (3)
   real(8), save :: WindVel (3)
   real(8), save :: ttop_acc(3)
   real(8), save :: psi     (3)
   real(8), save :: M_NRT_C (2,2) !NH,Tilt/yaw

   real(8), save :: collpit
!--- Demands
   real(8), save :: GenTrqDem   , PitAngDemColl                         ! Generator Torque and pitch angle from power control
   real(8), save :: GenTrqDTD(2), GenTrqDTDtot                          !
   real(8), save :: PITCH_INDIV(3), PITCH_INDIV_P(3), PITCH_INDIV_PP(3) ! Individual pitch angle demand
   real(8), save ::  FLAP_INDIV(3),  FLAP_INDIV_P(3),  FLAP_INDIV_PP(3) ! Individual flap  angle demand
   real(8), save :: GenTrqLSS_accel                                     ! Generator  Torque      for tower top acceleration reduction
   real(8), save :: BetaF_accel                                         ! Collective Flap  angle for tower top acceleration reduction
   real(8), save :: PITCH_accel                                         ! Collective Pitch angle for tower top acceleration reduction
!--- filtered vars
   real(8)       :: GenSpeed_LP1, collpit_LP1, power_LP1, GenTrq_LP1 !, Ux_LP1
   real(8)       :: GenSpeed_LP2, collpit_LP2, power_LP2, GenTrq_LP2 !, Ux_LP2

!--- Control parametes
!--- Power controller
   real(8), save :: power_rated, Q_rated
   real(8), save :: omega_min, omega_max
   real(8), save :: vs_Qmax, vs_Kopt
   integer, save :: vp_ipowtor                        !constant  power (1) or torque (2)
   real(8), save :: vp_pitmin, vp_pitmax, vp_pitratmin, vp_pitratmax
   real(8), save :: vs_Kp, vs_Ki, vs_Kd
   real(8), save :: vp_Kp, vp_Ki, vp_Kd, vp_Kp2, vp_Ki2
   real(8), save :: freq_free1
   real(8), save :: omega_set, vs_Qopt, vs_PI_Qmin, vs_PI_Qmax, vp_PI2_pitmin, vs_Qoptmin !vs_Qopt = vs_Kopt * w**2 !save for output
   real(8), save :: gearctrl
   real(8), save :: vs_Reg1_lim, vs_pit_switch
!--- Gain Schedule
   real(8), save :: GS, GS_NL, GS_IPC, GS_TTa, GS_NL_lim, GS_IPC0, GS_TTa0, kk1_inv, kk2_inv
!--- Drive Train Damper
   real(8), save :: DTD_gain(2), DTD_freq(2), DTD_damp(2)
...
!--- Filters
   real(8), save ::   LP2_freq(2),   LP2_damp(2)
   real(8), save :: Notch_freq(3), Notch_damp(3)
   real(8), save ::   LP1_lag (4)

   public :: lib_ctrl_init
   public :: lib_ctrl_main
   public :: lib_ctrl_update
...
!
!----------------------------------------------------------------------
 Subroutine lib_ctrl_init (array_in)
!----------------------------------------------------------------------

   implicit none

   real(8), intent(in) :: array_in(50)

   integer :: i, ii, iostat, nbod
   real(8) :: kk1, kk2
...
!--- Set input vars
   TIME            = array_in(    1)   !     1: general time                                 [s]
    GenSpeedLSS    = array_in(    2)   !     2: Rotor speed                                  [rad/s]
   dGenSpeedLSS    = array_in(    3)   !     3: Rotor acceleration                           [rad/s^2]
   BlPitch (1:3  ) = array_in( 4: 6)   !  4- 6: Blade pitch                                  [rad]
   WindVel (1:3  ) = array_in( 7: 9)   !  7- 9: Wind velocity [global c.s.]                  [m/s]
   ttop_acc(1:3  ) = array_in(10:12)   ! 10-12: Tower top acceleration                       [m/s^2]
   psi     (1:3  ) = array_in(13:15)   ! 13-15: Blade azimuth angle                          [rad]
   M_NRT_C (1,1:2) = array_in(16:17)   ! 16-18: Blade root moments at NR frame (tilt/yaw) 1P [Nm] 
   M_NRT_C (2,1:2) = array_in(18:19)   ! 19-21: Blade root moments at NR frame (tilt/yaw) 2P [Nm] 

!--- General Vars (set once)
   NBLADE          = int(array_in(40))
   DT_el           = array_in(   41)
   TPERIOD_el      = array_in(   42)
   RAT_GEAR        = array_in(   43)
   PI2             = 2.d0*dacos(-1.d0)
   R2D             = 360.d0/PI2

   if (NBLADE/=3) then; write(*,*)'Error in lib_ctrl_init, NBLADE /= 3'; stop; endif

!--- Set collective pitch for power control
!--- b_hGAST= -b_COLL -b_INDIV -b_accel + b_IMB  =>
!--- b_hGAST -b_IMB +b_COLL +b_INDIV +b_accel =0 =>
!---   -b_ctrl_in   +b_COLL +b_INDIV +b_accel =0 =>
!---   b_COLL =  b_ctrl_in  -b_INDIV -b_accel =0
      collpit  = 0.d0
   do nbod     = 1, NBLADE
      collpit  = collpit + (BlPitch(nbod)-PITCH_INDIV(nbod)-PITCH_accel) / dble(NBLADE)
   enddo
...
!------ power controller
      ...
      if (i==1) then
       ...
       GenSpeed       =  GenSpeedLSS * gearctrl
      dGenSpeed       = dGenSpeedLSS * gearctrl
...
!---11-14. LOWPASS-2nd (genspeed,collpitch,power,Torque)      [QTC ]      -//-                   NOT USED
!
...
!! QTC1_el (10 )     = WindVel(1)              !LP1
   ...
   TGenLSS           =  0.d0 !TREF * RAT_GEAR !T_HSS = T_LSS/RAT_GEAR
   TGenLSS_D         =  0.d0
   TGenLSS_M         =  0.d0
   TMLossLSS         =  0.d0
   TBrakeLSS         =  0.d0
!  TIME_BRAKE        = -1.d0
 ...
 END Subroutine lib_ctrl_init
!----------------------------------------------------------------------
 Subroutine lib_ctrl_main (array_in, array_out)
!----------------------------------------------------------------------

   implicit none

   real(8), intent(in   ) :: array_in (50)
   real(8), intent(  out) :: array_out(50)

   integer :: nbod

!--- Set input vars
   TIME            = array_in(    1)             !     1: general time                                 [s]
    GenSpeed       = array_in(    2) * gearctrl  !     2: Rotor speed                                  [rad/s]
   dGenSpeed       = array_in(    3) * gearctrl  !     3: Rotor acceleration                           [rad/s^2]
   BlPitch (1:3  ) = array_in( 4: 6)             !  4- 6: Blade pitch                                  [rad]
   WindVel (1:3  ) = array_in( 7: 9)             !  7- 9: Wind velocity [global c.s.]                  [m/s]
   ttop_acc(1:3  ) = array_in(10:12)             ! 10-12: Tower top acceleration                       [m/s^2]
   psi     (1:3  ) = array_in(13:15)             ! 13-15: Blade azimuth angle                          [rad]
...
!--- Set output vars
   array_out(    :) = 888.d10
   array_out(    1) = TGenLSS
   array_out(    2) = TGenLSS_D
   array_out(    3) = TGenLSS_M
   array_out( 4: 6) =   pitchOUT(1:3)
   array_out( 7: 9) =  dpitchOUT(1:3)
   array_out(10:12) = ddpitchOUT(1:3)
   array_out(13:15) =    flapOUT(1:3)
   array_out(16:18) =   dflapOUT(1:3)
   array_out(19:21) =  ddflapOUT(1:3)
   array_out(   22) = TMLossLSS
   array_out(   23) = TBrakeLSS
   

 END Subroutine lib_ctrl_main
!----------------------------------------------------------------------
 Subroutine lib_ctrl_update
!----------------------------------------------------------------------
...
#  endif
         sngl(TIME       )        ,&
        (sngl(QTC2_el (i)),i=1,NQSC0)
      close(27)

!--- Update and Output
   call IPC_control  (2)
   call TTACC_control(2)

 100  format(1000e15.5)


 END Subroutine lib_ctrl_update
!-----------------------------------------------------------------------------------------------
!--- Power controller ---
!
!--- Filters 1st and 2nd
...
!--- Generator's time lag for torque demand
   ...
   TGenLSS      =  QTC1_el (ieq)           * gearctrl
   TGenLSS_M    = (Q0M(1)+Q0M(2)) * Q0MLAG * gearctrl**2
   TGenLSS_D    =  0.d0
...
!--- Pitch Actuator: 2nd order (Pitch Angle demand)
   do nbod      = 1, NBLADE
      ...
      W         = freqPit_act      ![rad/s]
      D         = dampPit_act
      VAR       = PitAngDemColl + PITCH_INDIV(nbod) + PITCH_accel
      ...
      pitchOUT(nbod) = QTC_el  (ieq) !UT_el (i) = -QTC_el (ieq) + PITCH_Imb_el(nbod)
     dpitchOUT(nbod) = QTC1_el (ieq) !UT1_el(i) = -QTC1_el(ieq)
    ddpitchOUT(nbod) = QTC2_el (ieq) !UT2_el(i) = -QTC2_el(ieq)
   enddo !nbod
...
!--- Flap Actuator: 1st order low pass filter (usually 0.1s)
   do nbod      = 1, NBLADE
      ...
      flapOUT(nbod) =  QTC1_el (ieq)                     !  QTC1_el(ieq)                      * R2D
     dflapOUT(nbod) =  QTC2_el (ieq)                     !  QTC2_el(ieq)                      * R2D
    ddflapOUT(nbod) = (QTC2_el(ieq)-QTCP2_el(ieq))/DT_el ! (QTC2_el(ieq)-QTCP2_el(ieq))/DT_el * R2D
   enddo !nbod
...
!----------------------------------------------------------------------
   ...
   integer :: h, nn, nbod, ieq, ieq0
   real(8) :: GAIN   , CONP  , CONI
   real(8) :: VAR    , DVAR  , DDVAR
   real(8) :: W      , D     , W1,W2,D1,D2

   real(8)       :: theta   (NH,2), Bang(NB)
   real(8), save :: PVEL (NB), PACC(NB)  !saved for output
...
!------ 2nd-order low-pass filter at 2.5P
      ieq         = 15 + (h-1)*2 + nn-1 !15-18  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
...
!------ 2nd-order Notch filter at 3P
      ieq0        = ieq
      ieq         = 19 + (h-1)*2 + nn-1 !19-22  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      W1          = 3.d0*(PI2 / TPERIOD_el) ![rad/s]
      W2          = W1
      D1          = 0.001d0
      D2          = 0.200d0
      VAR         = QTC_el (ieq0)
...
!------ 2nd-order Notch filter at 2P
      ieq0        = ieq
      ieq         = 40 + (h-1)*2 + nn-1 !40-43  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      W1          = 2.d0*(PI2 / TPERIOD_el) ![rad/s]
      W2          = W1
      D1          = 0.001d0
      D2          = 0.200d0
      VAR         = QTC_el (ieq0)
...
!------ 2nd-order Notch filter at 1P
      ieq0        = ieq
      ieq         = 44 + (h-1)*2 + nn-1 !44-47  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      W1          = 1.d0*(PI2 / TPERIOD_el) ![rad/s]
      W2          = W1
      D1          = 0.001d0
      D2          = 0.200d0
      VAR         = QTC_el (ieq0)
...
!------ 2nd-order Notch filter at 6P
      ieq0        = ieq
      ieq         = 48 + (h-1)*2 + nn-1 !48-51  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      W1          = 6.d0*(PI2 / TPERIOD_el) ![rad/s]
      W2          = W1
      D1          = 0.001d0
      D2          = 0.200d0
      VAR         = QTC_el (ieq0)
...
!------ 2nd-order Notch filter at 1st tower
      ieq0        = ieq
      ieq         = 52 + (h-1)*2 + nn-1 !52-55  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      W1          = 0.25d0*PI2 ![rad/s]
      W2          = W1
      D1          = 0.001d0
      D2          = 0.200d0
...
!------ PI controller for Non-rotating M[tilt,yaw] at 1P & 2P freq.
      ieq0        = ieq
      ieq         = 23 + (h-1)*2 + nn-1 !23-26  Mtilt1P, Myaw1P, Mtilt2P, Myaw2P
      GAIN        = GS_IPC
      CONP        = ipc_kp(h)
      CONI        = ipc_ki(h)
      VAR         = QTC_el (ieq0)
...
!--- Output Individual Pitch Angle, Velocity and Acceleration [deg,deg/s,deg/s^2]
   if (ipc_PitchGain > 1.d-10) then
#    if   ASCII == 1
...
#    endif
        ...
        (sngl(psi        (nbod)*R2D)                          ,& !2, 6, 10
         ...
         sngl(PACC       (nbod)*R2D),    nbod = 1, NBLADE)    ,& !5, 9, 13
...
#    endif
        ...
        (sngl(psi        (nbod)*R2D)                          ,& !2, 6, 10
         ...
         sngl(FACC       (nbod)*R2D),    nbod = 1, NBLADE)    ,& !5, 9, 13
       ((sngl(M_NRT_C    (h    ,nn)),    nn=1,2) , h=1,ipc_nhT)  !14:tilt1P, 15:yaw1P, 16:tilt2P, 17:yaw2P
      close(23)
   endif
...
!!!real(8) :: AKSI0(3), xfl(3), dxfl(3), ddxfl(3), dxlfl(3), ddxlfl(3), accel0
   real(8), save :: accel, accelF !save for output
   integer :: ieq1, ieq2, ieq3
   real(8) :: W, D, W1,W2, D1,D2
   real(8) :: VAR, DVAR, DDVAR
...
!!!   call beam_kinematics (nbod,nbsub,nel,AKSI0,xfl,dxfl,ddxfl,dxlfl,ddxlfl)
!!!
!!!   accel            = ddxlfl(1)-accel0 !Atop-Abot

   accel            = ttop_acc(1) !global (x:fore-aft,y:side-side,z:vertical)
