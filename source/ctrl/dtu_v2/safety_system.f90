...
subroutine safety_system(stepno, deltat, omega, TTAccVect, EmergPitchStop, ActiveMechBrake, dump_array)
   ...
   if (omegafilt .gt. SafetySystemVar%overspeed) then
      EmergPitchStop = 1
      ActiveMechBrake = 1
      write(*,'(a,f6.2,a,f6.2,a)') 'Safety system alarm: Filtered generator speed = ',omegafilt,' rad/s. Threshold = ',SafetySystemVar%overspeed,' rad/s'
   endif
   !***********************************************************************************************
   ...
   if (TTAccFilt .gt. SafetySystemVar%RysteVagtLevel) then
      EmergPitchStop = 1
      ActiveMechBrake = 1
      write(*,'(a,f6.2,a,f6.2,a)') 'Safety system alarm: Filtered tower acceleration = ',TTAccFilt,' m/s^2. Threshold = ',SafetySystemVar%RysteVagtLevel,' m/s^2'
   endif
end subroutine
