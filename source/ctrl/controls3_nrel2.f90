...
REAL(8), PARAMETER      :: CornerFreq =   1.570796d0                       !#Corner frequency (-3dB point) in the recursive, single-pole, low-pass filter, rad/s. -- chosen to be 1/4 the blade edgewise natural frequency ( 1/4 of approx. 1Hz = 0.25Hz = 1.570796rad/s)
...
REAL(8), PARAMETER      :: MIN_Spd    =  70.16224d0    !  670.0rpm         !#Transitional generator speed (HSS side) between regions 1     and 1 1/2, rad/s.
REAL(8), PARAMETER      :: MAX_Spd    = 122.9096d0     ! 1173.7rpm         !#Desired (reference) HSS speed for pitch controller, rad/s.
...
REAL(8), PARAMETER      :: VS_RtPwr   = 5296610.d0         ! 5MW/0.944     !#Rated generator power in Region 3, Watts. -- chosen to be 5MW divided by the electrical generator efficiency of 94.4%
REAL(8), PARAMETER      :: VS_Rgn2K   =       2.332287d0                   !#Generator torque constant in Region 2 (HSS side), N-m/(rad/s)^2.
REAL(8), PARAMETER      :: VS_pow     =       2.0d0                        !#Generator torque omega power constant in Region 2 (Default is 2).
...
!--- PID2
integer, save :: IPID2 = 0    !qq       !flag to enable the PID2 option
real(8), save :: error (1)
...
!qq
     if (IPID2==1) then
      PC_KI      = 0.008068634d0 !* 3.d0
      PC_KP      = 0.01882681d0  !* 3.d0
     endif
   endif
