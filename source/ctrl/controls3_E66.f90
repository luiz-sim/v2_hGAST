...
REAL(8), PARAMETER      :: CornerFreq =   1.00d0                           !#Corner frequency (-3dB point) in the recursive, single-pole, low-pass filter, rad/s. -- chosen to be 1/4 the blade edgewise natural frequency ( 1/4 of approx. 1Hz = 0.25Hz = 1.570796rad/s)
...
REAL(8), PARAMETER      :: PC_KK      =   0.066d0      !     3.78deg     ??!#Pitch angle were the derivative of the aerodynamic power w.r.t. pitch has increased by a factor of two relative to the derivative at rated pitch (zero), rad.
...
REAL(8), PARAMETER      :: MIN_Spd    =   8.5d0 /RPS2RPM                   !#Transitional generator speed (HSS side) between regions 1     and 1 1/2, rad/s.
REAL(8), PARAMETER      :: MAX_Spd    =  22.5d0 /RPS2RPM                   !#Desired (reference) HSS speed for pitch controller, rad/s.
...
REAL(8),       SAVE     :: PC_RefSpd  = MAX_Spd            ! 1173.7rpm     ! Desired (reference) HSS speed for pitch controller, rad/s.
...
REAL(8), PARAMETER      :: VS_RtPwr   = 1978700.d0         !~1860/0.94     !#Rated generator power in Region 3, Watts. -- chosen to be 5MW divided by the electrical generator efficiency of 94.4%
REAL(8), PARAMETER      :: VS_Rgn2K   =  151270.d0         !142193         !#Generator torque constant in Region 2 (HSS side), N-m/(rad/s)^2.
...
REAL(8),      SAVE      :: VS_Rgn2Sp  = MIN_Spd   * 1.3d0                  ! Transitional generator speed (HSS side) between regions 1 1/2 and 2    , rad/s.
...
REAL(8),      SAVE      :: res_vs(8), res_pc(26)                           ! Store resutls for output
...
integer, save :: cutin_step     =  0    !cutout phase 0,1,2
!--- PID2
...
!  call lint1  ( xa, ya, n1, n2, np, x     , y         )
   call lint1  ( V , P , 1 , 4 , 9 , Velhub, PC_MinPit )             ! PC_MinPit: Minimum pitch setting in pitch controller, rad.
...
!---------------------------------
 ...
 case (0)  ! Initialization
   ...
   if (icase >= 3) then
      ...
      PC_KI      = 0.13d0 !0.0008965149d0 !REDUCE GAINS FOR HYWIND
      PC_KP      = 0.37d0 !0.006275604d0  !REDUCE GAINS FOR HYWIND
   else
      PC_KI      = 0.508d0         !1.3d0 !0.008068634d0
      PC_KP      = 1.520d0        ! 1.1d0 !0.01882681d0 
!qq
     if (IPID2==1) then
      PC_KI      = 0.508d0 * 3.d0! !1.3d0 !0.008068634d0
      PC_KP      = 1.520d0 * 3.d0!  1.1d0 !0.01882681d0 
     endif
   endif
...
!qq
   ...
   V(2)= 10.d0 ;  P(2)= 0.00/R2D;
   V(3)= 15.d0 ;  P(3)= 0.00/R2D; !P(3)= 9.40/R2D;
   V(4)= 55.d0 ;  P(4)= 0.00/R2D; !P(4)= 9.40/R2D;
...
!        1      2       3     4          5          6       7     8
!      Time,GenTrq,TrqRate,LastGenTrq,GenSpeedF,GenSpeed,Alpha,Region
     open (400,file='gen_control_VS.dat',access='append')
      write(400,10) (res_vs(I),I=1,8), dble(cutin_step), dble(cutout_step)
     close(400)
