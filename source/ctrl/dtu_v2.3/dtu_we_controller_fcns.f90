module dtu_we_controller_fcns
   ...
   type Tpidvar
      ...
      integer stepno1
      real(mk) outset,outpro,outdif,error1_old,outset1_old,outres1_old,outres, INIT_OUT !j
   end type Tpidvar
   type Tpid2var
      real(mk) Kpro(2),Kdif(2),Kint(2),outmin,outmax,velmax,error1(2),outset1,outres1
      integer stepno1
      real(mk) outset,outpro,outdif,error1_old(2),outset1_old,outres1_old,outres, INIT_OUT !j
   end type Tpid2var
   type Twpdata
...
!!jelseif (x0 .gt. x1) then
!!j   switch_spline = 0.0_mk
   else
      if (x .lt. x0) then
         switch_spline = 0.0_mk
      elseif (x .gt. x1) then
...
!!j      switch_spline = 2.0_mk/(-x1 + x0)**3*x**3 + (-3.0_mk*x0 - 3.0_mk*x1)/(-x1 + x0)**3*x**2&
!!j                      +6.0_mk*x1*x0/(-x1 + x0)**3*x + (x0 - 3.0_mk*x1)*x0**2/(-x1 + x0)**3
      endif
   endif
   return
...
function PID(stepno, dt, kgain, PIDvar, error)
   ...
   ! Initiate
   if (stepno.eq.1) then
      PIDvar%outset1=0.0_mk + PIDvar%INIT_OUT !j
      PIDvar%outres1=0.0_mk + PIDvar%INIT_OUT !j
      PIDvar%error1 = 0.0_mk
      PIDvar%error1_old = 0.0_mk
      PIDvar%outset1_old=0.0_mk + PIDvar%INIT_OUT !j
      PIDvar%outres1_old=0.0_mk + PIDvar%INIT_OUT !j
   endif
