module turbine_controller_mod
   ...
   implicit none
   ! Parameters
   logical , save :: generator_cutin
   integer , save :: PartialLoadControlMode, stoptype
   real(mk), save :: deltat
   real(mk), save :: GenSpeedRefMax, GenSpeedRefMin, PeRated, GenTorqueRated, PitchStopAng, GenTorqueMax
   real(mk), save :: TTfa_PWR_lower, TTfa_PWR_upper, TorqueCtrlRatio
   real(mk), save :: Kopt, Kopt_dot, TSR_opt, R, GearRatio
   real(mk), save :: Vcutout, Vstorm
   real(mk), save :: Err0, ErrDot0, PitNonLin1, rel_limit
   integer , save :: NAve_Pitch
   real(mk), save :: PitchAngles(1000,3),PitchRefs(1000,3)
   real(mk), save :: TAve_Pitch,DeltaPitchThreshold,AveragedMeanPitchAngles(3),AveragedPitchReference(3)
   ! Dynamic variables
   integer , save :: stepno = 0, w_region = 0
   real(mk), save :: AddedPitchRate, PitchColRef0, GenTorqueRef0, PitchColRefOld, GenTorqueRefOld
   real(mk), save :: TimerGenCutin, TimerStartup, TimerExcl, TimerShutdown, TimerShutdown2
   real(mk), save :: GenSpeed_at_stop, GenTorque_at_stop
   real(mk), save :: excl_flag
   real(mk), save::outmax_old=0.0_mk,outmin_old=0.0_mk !qq save added by jim
   logical::fullload=.false.
   integer::CountPitchDeviation=0
   ! Types variables
   type(Tlowpass2order), save :: omega2ordervar
!v2type(Tfirstordervar), save :: switchfirstordervar
...
subroutine turbine_controller(CtrlStatus, GridFlag, GenSpeed, PitchVect, wsp, Pe, TTAccVect, &
   ...
   if (CtrlStatus .eq. 0) then
      call normal_operation(GenSpeed, PitchVect, wsp, Pe, TTAccVect(2), GenTorqueRef, PitchColRef,&
                            dump_array)
 write(44,*)"stop0",dump_array(50),CtrlStatus;!stop
   endif
   !***********************************************************************************************
 ...
 write(44,*)"stop1",dump_array(50),CtrlStatus;!stop
      else
 write(44,*)"stop2",dump_array(50),CtrlStatus;!stop
 ...
 write(44,*)"stop3",dump_array(50),CtrlStatus;!stop
      call shut_down(CtrlStatus, GenSpeed, PitchVect, wsp, GenTorqueRef, PitchColRef, dump_array)
   endif
   !***********************************************************************************************
...
!j GenTorqueRef = min(max(GenTorqueRef + Qdamp_ref*x, 0.0_mk), GenTorqueMax)
...
!qqv2 ! Start up the switch filter
!qq   dummy = lowpass1orderfilt(deltat, stepno, switchfirstordervar, 0.0_mk)
      ! Windup the integral term of PID controller
      PID_gen_var%outmin = GenTorqueRef
      kgain_torque = 1.0_mk
      dummy = PID(stepno, deltat, kgain_torque, PID_gen_var, GenSpeedFiltErr)
...
i2=3
   ...
   dump_array(20) = PitchMeanFilt
write(46,'(100e16.6)') GenTorqueRef,GenTorqueRef0,PitchColRef,PitchColRef0, dble(PartialLoadControlMode),i1,i2
!                          1             2              3             4                       5           6  7
...
subroutine shut_down(CtrlStatus, GenSpeed, PitchVect, wsp, GenTorqueRef, PitchColRef, dump_array)
   ...
   dump_array(18) = CtrlStatus
   dump_array(20) = PitchMeanFilt
write(45,'(100e16.6)') PitchColRef,PitchColRefOld, PitchStopAng, CutoutVar%pitchvelmax, deltat, PitchMeanFilt
...
subroutine monitoring(CtrlStatus, GridFlag, GenSpeed, TTAcc, PitchVect, PitchColRef, dump_array)
   ...
   if (DeltaPitchThreshold*TAve_Pitch.gt.0.0_mk) then
     if (NAve_Pitch.gt.1000) then
        write(*,'(a)') 'ERROR: Length of time window for pitch angle averaging in the pitch deviation monitor is too long (>1000 time steps)'
        stop
     else
       ! Counter
...
!j ConstantPowerTorque=min((GenTorqueRated*GenSpeedRef_full)/max(GenSpeed    , GenSpeedRefMin), GenTorqueMax)
   ConstantPowerTorque=min((GenTorqueRated*GenSpeedRef_full)/max(GenSpeedFilt, GenSpeedRefMin), GenTorqueMax)
...
!      GenSpeed_max2 = SwitchVar%rel_sp_open_Qg*GenSpeedRefMax
       GenSpeed_min1 = GenSpeedRefMin
       GenSpeed_min2 = GenSpeedRefMin * 1.05_mk
       GenSpeed_max1 = (SwitchVar%rel_sp_open_Qg - 0.05_mk)*GenSpeedRefMax
       GenSpeed_max2 = SwitchVar%rel_sp_open_Qg*GenSpeedRefMax
...
!##v2  GenTorqueMax_partial = max(GenTorqueMax_partial, Kopt*GenSpeed_min2**pow)
   dump_array(43) = GenTorqueMin_partial
   dump_array(44) = GenTorqueMax_partial
...
!qqPID_gen_var%velmax       = GenTorqueMax/(array1(33)+0.001_mk) !was 0. in ver2.0
...
!  SwitchVar%rel_sp_open_Qg = array1(35)*0.01_mk
   ...
   if (PitchMean.le.switch_pitang_lower) fullload=.false.
   if (PitchMean.ge.switch_pitang_upper) fullload=.true.
   if (.not.fullload) then
     switch = switch_spline(PitchMean, PitchMin, switch_pitang_upper)
!qqv2switch = lowpass1orderfilt(deltat, stepno, switchfirstordervar, switch)
   else
     switch=1.0_mk
   endif
   ...
   !-----------------------------------------------------------------------------------------------
!##if (stepno>1) then !qq jim added if
...
!qqwrite(3332,'(10e15.5)') Kgain(1,1),Kgain(2,1),PID_pit_var%kpro(1),&
...
subroutine drivetraindamper(GenSpeed, Qdamp_ref, dump_array)
   ...
   !
   real(mk), intent(in)  :: GenSpeed  ! Measured generator speed [rad/s].
   real(mk), intent(out) :: Qdamp_ref ! Generator torque reference component from the drivetrain damper [Nm].
   real(mk), intent(inout) :: dump_array(50) ! Array for output.
...
subroutine towerdamper(TTfa_acc, theta_dam_ref, dump_array)
   ...
   real(mk), intent(out)   :: theta_dam_ref ! Reference pitch angle component from longitudinal tower damper [rad].
   real(mk), intent(inout) :: dump_array(50) ! Array for output.
